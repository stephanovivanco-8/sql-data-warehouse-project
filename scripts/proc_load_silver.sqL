USE [DataWarehouse];  -- cambiar a la base de datos correcta
GO
...
BEGIN
    ...
    BEGIN TRY
		...
		SELECT 
			...
			END AS sls_due_dt,
			CASE 
				WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantify * ABS(sls_price) 
					THEN sls_quantify * ABS(sls_price)
				ELSE sls_sales
			END AS sls_sales, -- Recalculate sales if original value is missing or incorrect
			sls_quantify,
			CASE 
				WHEN sls_price IS NULL OR sls_price <= 0 
					THEN sls_sales / NULLIF(sls_quantify, 0)
